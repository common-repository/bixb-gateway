...
function bixb_add_gateway_class( $gateways ) {
	$gateways[] = 'Bixb_Gateway'; // your class name is here
	return $gateways;
}
...
function bixb_init_gateway_class() {
	...
	class Bixb_Gateway extends WC_Payment_Gateway {
		 ...
		 */
	 	public function payment_scripts() {
			...
			if ( ! is_cart() && ! is_checkout() && sanitize_text_field( $_GET['pay_for_order'] ) !== null ) {
				return;
			}
		...
		public function thankyou_page($order_id) {
			...
			$address = Bixb_Gateway::get_order_address($order_id);
			$amount = Bixb_Gateway::get_bixb_total( $order_id );
			?>
			...
			<?php _e( Bixb_Gateway::get_qr_image($address, $amount) ); ?></h4>
			<?php
		}
